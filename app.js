...
let rate=0;
let productname
function c( num,num1,Rate,productn)
{
...
localStorage.setItem("cartolddata",cartitemdiv.innerHTML);
putcartdaat();
let flage=0;
function removecartdata()
{
    ...
    localStorage.setItem("cartdata",localStorage.getItem("cartolddata"));
    remocart.style.display="none";
    flage = 1;
    putcartdaat();
}
let gtotal=0;
...
function additem()
{
    ...
    divv.innerHTML+=productname;

    divv.appendChild(minediv);
    minediv.innerHTML+=rate;
    ...
    span.innerHTML=`X`;
    remocart.style.display="block";
    remocart.style.display="flex";
     to = localStorage.getItem("amount")*100;
     var options ={
      "amount": to,
    };
    console.log("lal");
    ...
    else
    {
      ...
      minediv.innerHTML+=rate;
      const span = document.createElement("button");
      totalamount=Number(localStorage.getItem("amount"))+totalamount;
      localStorage.setItem("amount",totalamount);
      console.log(totalamount)
      divv.appendChild(span);
      span.innerHTML=`X`;
